...
async def get_form(request: Request, config: ResourceConfig):
    dt_start = datetime.combine(date.today(), config.day_start_time)
    dt_end = datetime.combine(date.today(), config.day_end_time)
    time_slots = [dt_start]

    while (
        next_dt := time_slots[-1] + timedelta(minutes=config.minutes_increment)
    ) <= dt_end:
